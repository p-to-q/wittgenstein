...
This is a *plan*, not the measurement. The actual measurement requires:
- Local Python environment with PhysioNet / UEA archive datasets downloaded.
- A small classifier training stack (sklearn / PyTorch).
- Spectral analysis tooling (scipy.signal).
- A few hours of compute.

I am writing the plan; the partner or a future maintainer runs it.
...
**Pre-commitment.** If H0 is not rejected by EITHER pass criterion above, patchGrammar stays permitted-but-not-promoted. We do NOT keep adding operators or running variants until something passes. The bar is a one-shot test against pre-registered criteria.
...
### Temperature — UCI ML repository: Indoor Localization or HAR thermal subset

**Citation.** UCI HAR (Anguita et al., *"A Public Domain Dataset for Human Activity Recognition Using Smartphones,"* ESANN 2013) — primarily accelerometer/gyro but includes thermal-style time-series subsets. Dataset DOI varies; CC-BY-4.0 in most publications.

**Caveat.** Pure temperature-signal datasets with permissive licenses are rarer than ECG. Alternative: synthetic temperature-from-noise from public weather data (NOAA hourly stations have CC0 license; pick one station's 24h temperature trace).

**For the measurement.** Spectral distance against a NOAA station's 24h trace; data-augmentation against a small temperature-classification task (e.g. "cold-snap detection").

### Gyro — UCI HAR (gyro subset)

**Same dataset citation as above.** UCI HAR provides 3-axis gyroscope traces during 6 human activities. License CC-BY-4.0 per typical UCI release.
...
- **Dataset SHAs pinned** in the results note.
