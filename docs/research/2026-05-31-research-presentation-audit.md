...
| M1B threshold policy                           | `decision-coded by #473` | `docs/research/2026-05-31-m1b-gate-acceptance-policy.md`, `packages/codec-image/src/decoders/manifest.ts`, `research/validation/vqgan_gate_audit.py`, #473, #474                                                                                                                                      | Gate C/D acceptance is now manifest-declared. LlamaGen structural parity keeps re-encode token drift advisory; Gate D shape/latency remain hard checks through the decoder-family manifest.                                                               |
...
1. Updated `research/validation/fixtures/m1b-audit/gate-c-pass.fixture.json`
   so the fixture actually satisfies the Gate C hard floor then follow-up #473
   moved that floor to manifest-declared acceptance:
   `encode_consistent=true`, `decode_consistent=true`, `sample_count=3`, and
   matching `cross_device_parity`.
