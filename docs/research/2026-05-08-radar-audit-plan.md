...
The plan also makes one thing honest that the radar didn't: **without web access, I cannot directly inspect LICENSE files or fetch HuggingFace model cards**. The radar tagged these claims `verify-locally` (citing #238). This audit plan is what the verification looks like *when someone with web access runs it*, plus a reproducibility scaffold so the audit doesn't drift when re-run later.
...
| Gate | Status | Specific verifiers (web access required) |
|---|---|---|
...
- Does NOT do the audits themselves. Web access required.
