...
- `adapters/seed-expander.ts`
  ...
  - `tileMosaicSeedExpander` (PR #252) — a second deterministic implementation that exercises the seam as an ABI peer rather than a refactor; uses a 2D coarse mosaic instead of linear modulo.
  - `blockCausalMaskgitSeedExpander` (issue #453 closeout) — a deterministic, placeholder-class block-causal / MaskGIT scheduling surface inspired by the Cola-DLM notes. It preserves clean-repaint pins, reveals unknown tokens over an 8-step mask schedule, and records `block-causal-maskgit-expander/v0` when selected. This is an executable ABI baseline, not the trained L4 adapter; learned quality remains owned by `research/training/adapter/` and #397.
  - Runtime selection is explicit through `WITTGENSTEIN_IMAGE_SEED_EXPANDER`: `placeholder` (default), `tile-mosaic`, or `block-causal-maskgit`. Unknown values fail loudly rather than silently falling back.
- `pipeline/decoder.ts`
